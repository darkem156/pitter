...
{
  const { id } = req.params;
  const data = (await User.findOne({ where: { id } })).dataValues
  if (!data) res.status(404).json({ name: "Este Usuario No Existe" })
...
const follow = async (req, res) =>
{
  ...
  if(!id_user) res.status(401).json({ error: "Debes iniciar sesion para seguir a un usuario" })
  else if(id_user === id) res.status(409).json({ followed: "no te puedes seguir a ti mismo" });
